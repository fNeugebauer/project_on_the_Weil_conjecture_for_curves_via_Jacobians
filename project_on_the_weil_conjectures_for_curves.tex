...
%	as $\mathbb{Q}$-algebras.
	...
	For $f\in \End(X)$ we define $\deg f$ to be the degree of $f$ if $f$ is an isogeny and zero otherwise. Because the degree is multiplicative we can extend this to $\deg:\End^0(X)\to \Q$ via $\deg(\frac{f}{n}):=n^{-2g}\deg{f}$. 
 	...
 	\end{cor}
 	\begin{proof}
 	For an abelian variety $X$ the $\Z_l$-module $T_l X$ is free of rank $2\dim X$ and therefore $\Hom_{\Z_l}(T_l X, T_l Y)$ is free of rank $4\dim X \dim Y$. Since $\Z_l$ is a principal ideal domain we can conclude from theorem \ref{thm injec} that $\Z_l \otimes \Hom(X,Y)$ is a free $\Z_l$ module of rank $\le 4 \dim X \dim Y$.  This bounds the rank of the torsion free abelian group $\Hom(X,Y)$ by $4\dim X \dim Y$. 
 	\end{proof}
 	...
 	In this section $C$ shall be a non-singular proper curve of genus $g$ over a field $k$. Curves are assumed to be geometrically integral one-dimensional schemes of finite type and separated over $k$. 
...
\begin{thm}[Weil conjectures for curves]\label{thm Weil conjectures for curves} ~\\
	Let $P$ be the characteristic polynomial of $\text{Fr}\in \End(J_k)$ and $\alpha_1, \dots, \alpha_{2g} \in \mathbb{C}$ its roots. The Weil conjectures for curves state:
	\begin{enumerate}
...
\begin{proof} Let $F_k: \Spec(k)\to\Spec(k)$ denote the absolute Frobenius of $k$. 
	...
	\end{align}
	$\# C(\mathbb{F}_{q^n})=\Delta_C.\Gamma_{F_{C,k}^n}$. For the $k$-variety $C_k$ we have a bijection between the $k$-valued points of $C_k$ and the $k$-valued points of the $\mathbb{F}_q$ variety $C$ given by
...
\textbf{Claim 2}:	The bijection in (\ref{eq bijection}) identifies both actions a) and b):\\ Let $x\in C_k(k)$. Then $\pr_C\circ F_{C,k} \circ x=F_C \circ \pr_C \circ x=F_S\circ \pr_C \circ x$, where we used the naturality of the absolute Frobenius in the last equation.
	...
	\end{align}
	We want to apply proposition \ref{prop fixed points and intersection} to $F^n_{C,k}: C_k \to C_k$. 
	
	For a) note that $\#C(\mathbb{F}_{q^n})<\infty$ because $C$ admits a closed immersion into $\mathbb{P}^m_k$ for $m$ big enough and $\mathbb{P}^m_k(\mathbb{F}_{q^n})=((q^n)^{m+1}-1)/(q^n-1)<\infty$. So, by equation \ref{eq fixed points froben} $F_{C,k}^n(x)=x$ for only finitely many $x\in C_k(x)$. 
	
	For b) observe that for $x\in C_k(k)$ we have $(\dd F_{C,k})_x =0$, because $q^n=0$ in $k$. This can be proven very explicitly by using an étale morphisms as in the proof of claim 1. Then  $(\dd F_{C,k})_x =0$ follows because the endomorphism of $\mathbb{A}_{F_q}$ that fixes the coefficients and raises the coordinates to their $q^n$ power induces the zero map on tangent spaces. 
	
	All in all, we can apply proposition \ref{prop fixed points and intersection} to obtain
	\begin{align}\#C(\mathbb{F}_{q^n})\overset{(\ref{eq fixed points froben})}{=}\#\{x \in C_k(k): F_{C,k}^n(x)=x\}=\Delta_C.\Gamma_{F_{C,k}^n}\overset{(\ref{eq intersection frobenius diagonal})}{=}1-\sum_{i=1}^{2g}\alpha_i^n + q^n. \label{eq points of C in proof}
	\end{align}  
	For showing the Riemann hypothesis for the curve $C$ we proceed to  proof the following statement.\\
...
Let $0\neq a \in \Q[\Fr]$. Then $b:=a^\dagger \cdot a$. By the positivity of the Rosati involution \ref{cor Positiviy of the Rosati involutionn} $\tr(b)=\tr(a^\dagger \cdot a)>0$. So, $b\neq 0$. As $b^\dagger = b$ also $\tr(b^2)=\tr(b^\dagger b)>0$ by the positivity of the Rosati involution \ref{cor Positiviy of the Rosati involutionn} and hence $b^2\neq 0$. Similarly, $b^4\neq 0$ and by induction $b^{2n}\neq 0 $. Hence $b$ is not nilpotent. Because $\Q[\Fr]$ is commutative, if $a$ was nilpotent, then also $b$ would be nilpotent. We conclude that $\Q[\Fr]$ is reduced. 
...
Now $\# J(\mathbb{F}_{q^n})=\# J_{\mathbb{F}_{q^n}}(\mathbb{F}_{q^n})$ and the latter equals $\# \Pic^0(C_{\mathbb{F}_{q^n}})$ whenever $C_{\mathbb{F}_{q^n}}$ has an $\mathbb{F}_{q^n}$-valued point by equation (\ref{eq 9}) and the discussion in section \ref{sec induced map} b) because $J_{\mathbb{F}_{q^n}}=J\times_{\mathbb{F}_q}\mathbb{F}_{q^n}$ is the Jacobian variety of the curve $C_{\mathbb{F}_{q^n}}$ over $\mathbb{F}_{q^n}$. 
