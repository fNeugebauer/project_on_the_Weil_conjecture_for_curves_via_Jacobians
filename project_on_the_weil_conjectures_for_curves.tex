...
\title{The Weil conjecture for curves via Jacobian varieties}

\author{Kai Fabio Neugebauer}
...
\begin{document}
	\maketitle
	\begin{tabular}{ll}
		Aufgabensteller: & Prof. Dr. Christian Liedtke\\
		Betreuer:  & Marius Kjærsgaard 
	\end{tabular}
	\begin{abstract}
		The most straightforward way to prove the Weil conjectures for a curve $C$ over a finite field $k$ is via intersection theory on the surface $C\times C$ as in \cite[V Exerc. 1.10]{hartshorne2013algebraic}. We present a different approach, which is historically in the spirit of Weil's original proof. The idea is to associate to $C$ an abelian variety $J$, called the Jacobian of $C$. To obtain the Weil conjectures we relate the fixed points of the Frobenius endomorphism of $C$ to the trace of the induced endomorphism of $J$.
		
		In the following we will develop the theory of Abelian and Jacobian varieties enabling us in the end to prove the Weil conjectures for curves.  
	\end{abstract}
	\tableofcontents
	...
	We will use the notations $\Om_X(D)=\Om(D)$ for the sheaf associated to an effective Cartier divisor $D$ on $X$, i.e. $\Om(D)=I_D^{-1}$. $\Li_{X}(D)=\Li(D)$ will denote the invertible sheaf associated to a Weil-Divisor $D$ on $X$. 
	...
	\begin{proof}
		...
		Applying the previous paragraph to $f\vert_{X\times \{P\}}$ (note: $X\cong X\times \{P\}$) we conclude that $f(X\times \{P\})=g(P)$. 		
		 ...
		 We have shown that the set of points where $f=g\circ \pr_Y$ contains $\bigcup_{P\in (X\setminus V)(k)} X\times \{P\}$. Because $k=k_s$ the latter set is dense in $X\times Y$ and we are done by \cite[Sect. 10.2.A]{vakil2017rising}.
	\end{proof}
	...
	\begin{cor}\label{cor morphism between abelian varieties factor as homomorphism}
		...
		by $f(0)$ on $Y$.
	\end{cor}
	\begin{proof}Let $y=-f(e_X)$ and let $h=t_y \circ f$. Define $g: X\times X \to Y$ to be the map that one points is given by $g(x,x')=h(x+x')-h(x)-h(x')$, i.e. $g=m_Y\circ( h\circ m_X, m_Y\circ (((-1)_Y\circ h )\times ((-1)_Y\circ h)))$. Then 
		\[g(\{e_X\}\times X)=g(X\times \{e_X\})=-h(e_X)=\{e_Y\}\]
	...
	\begin{proof}
		...
		Assume that $f$ is surjective. By \cite[Thm. 11.4.1]{vakil2017rising} there exists a nonempty open subset $U\subseteq Y$ such that that for all $q\in U$ the fiber over $q$ has pure dimension $\dim X - \dim Y$. By the above, $\dim \ker f = \dim X - \dim Y$. This proves a)$\implies$ b) $\implies $ c).
		...
		For any $q\in Y$ there is an affine open neighborhood $U =\Spec R$ such that $(f_* \Om_X)\vert_U \cong \Om_Y^d \vert_U$. $f$ is finite and therefore affine, so $f^{-1} U = \Spec( R')$ for some ring $R'$. Then $f^\#_U: R \to R'$ makes $R'$ a free $R$ module of rank $d$ and $f^{-1}(q)\cong \Spec(R'\otimes_R k(q))$ proves that \begin{align}
		...
		\end{thm}
		\begin{proof}
			Both parts of the theorem can be seen as corollaries of the \textit{theorem of the cube}, which is a theorem on proper varieties. References are \cite[Chp. II §1]{van2007abelian} or \cite[Chp. II.6]{Mum74}.
		\end{proof}
		...
		\begin{lem}\label{lem pullbacks along multiplication}
			For any line bundle $\Li$ on an abelian variety $X$ and $n \in \mathbb{Z}$ 
			\[n_X^*\Li \cong \Li^{n(n+1)/2}\otimes(-1)^*\Li^{n(n-1)/2}.\]
			In particular, if $\Li$ is symmetric, i.e. $\Li \cong (-1)^*\Li$, then $n_X^*\Li \cong \Li^{n^2}$. 
		\end{lem}
		\begin{proof}
			Apply equation \ref{eq cube} from theorem \ref{thm of the cube and the square} for $f=n, g=1 $ and $h=-1$ to obtain
			\begin{align*}
				n^*\Li \cong (n+1)^*\Li \otimes (n-1)^*\Li \otimes n^*\Li^{-1}\otimes (-1)^*\Li^{-1}\otimes \Li^{-1}
			\end{align*}
		and therefore 
		\begin{align*}
			(n+1)^*\Li \otimes (n-1)^*\Li\cong n^*\Li^2 \otimes  (-1)^* \Li \otimes \Li.
		\end{align*}
		The assertion now follows from induction, by first checking the cases $n=-1,0,1$ by hand.
		\end{proof}
		\begin{proof}[Proof of proposition \ref{torsion points of abelian varieties}]
			 By remark \ref{rmk abelian varieties are projective} there exists a ample line bundle $\mathcal{L}$ on $X$. We can assume $\mathcal{L}$ to be symmetric, i.e. $(-1)^*\mathcal{L}\cong \mathcal{L}$, because when $\mathcal{L}$ is ample then also  $(-1)^*\mathcal{L}\otimes \mathcal{L}$ will be ample by \cite[II Ex. 7.5 (c)]{hartshorne2013algebraic}. By lemma \ref{lem pullbacks along multiplication}  $n_X^*\Li\cong \Li^{n^2}$, so $n_X^*\Li$ is an ample line bundle provided that $n^2>0$. Its pullback $\iota^*\n_X^*\Li$ along the closed immersion $\iota:X[n]\to X$ will also be an ample line bundle. But $n_X \circ \iota$ factors through the zero map and therefore $\iota^*n_X^* \Li$ is trivial.
			 
			 We proceed to prove that a proper variety admitting a trivial ample line bundle is finite: 
			 
			 By \cite[\href{https://stacks.math.columbia.edu/tag/01QE}{Tag 01QE}]{stacks-project} $X[n]$ is quasi-affine. Hence the canonical map $X[n]\to \Spec(\Gamma(X[n], \Om_{X[n]}))$ is an open immersion. But $X[n]$ is proper over $k$, so this open immersion is moreover proper and therefore a closed immersion. This proves that $X[n]$ is a proper and affine variety, and therefore finite as asserted. By theorem \ref{thm isogenies} c) $n_X$ is an isogeny.
 		...
 		Hence $\pr_T^*:\Pic(T)\to \Pic(X_T)$ is a section to the pullback $\varepsilon_T^*: \Pic(X_T)\to \Pic(T)$ along $\varepsilon_T$, 
...
% 	\end{align}
 	...
 	\subsubsection{The dual abelian variety}\label{sec dual abelian}
 		...
 		\end{lem}
 		\begin{proof} 
 			Let $T=K(\Li)$ and $x: K(\Li)\to X$ be the inclusion. Then $\Lambda(\Li)\vert_{X \times K(\Li)}=(1\times x)^*\Lambda(\Li)$ represents $\varphi_\Li\circ x$, which is trivial by definition of $K(\Li)$. 
 			
 			For (ii) let $\Li$ be an ample line bundle on $X$. Then its pullback $\Li'$ to $K(\Li)$ is ample because $x: K(L)\to X$ is a closed immersion. By (i) the bundle $(1\times x)^*\Lambda(\Li)$ is trivial on $X\times  K(\Li)$. Hence also $(x,(-1))^*(1\times x)^*\Lambda(\Li)\cong \Li'^{-1}\otimes (-1)^* \Li'^{-1}$ is trivial on $K(\Li)$. So, $\Li'\otimes (-1)^*\Li'$ is an ample and trivial sheaf on the closed subscheme $K(\Li)$ of the proper scheme $X$.
	...
	\begin{proof}
		...
		Hence $\Pic_{X/k}$ is an abelian variety by (\ref{thm tangent space of Picard Functor}) and the discussion above (\ref{thm tangent space of Picard Functor}). $\varphi_{\mathcal{L}}$ will be an isogeny by theorem (\ref{thm isogenies}) using that its kernel is finite by lemma \ref{the kernel of polar} (ii).
	\end{proof}
	...
	For  $x\in Y^\vee(T)$ represented by $\Li \in \Pic(Y_T)$ we obtain from proposition \ref{prop poincare} that $f^\vee(x)$ is represented by 
	\begin{align}(1\times f^\vee \circ x)^*\Po_X=(1\times x)^*(f\times 1)^*\Po_Y =(f\times 1)^*(1\times x)^*\Po_Y=(f\times 1)^*\Li. \label{eq dual map on points}
	\end{align} 
	\subsection{Endomorphisms of abelian varieties}
	In this chapter $X$ and $Y$ will be abelian varieties over the field $k$, $X$ will have dimension $g$ and $l$ will be a prime number not equal to $\text{char}(k)$. We give a proof of the Poincaré Splitting Theorem (\ref{thm poincare splitting}). 
	
	
	\begin{proof}
		Let $\iota: Y \to X$ be the inclusion and $\lambda: X \to X^\vee$ a polarization. 
		
		For $K:=\ker( X \xrightarrow{\lambda}X^\vee \xrightarrow{\iota^\vee} Y^\vee)$ define $Z$ to be the connected components of $K$ with its reduced subscheme structure. Then $Z$ is an abelian variety of dimension $ \dim X - \dim Y$. By \cite[Exerc. 11.1]{van2007abelian} $\iota^\vee \circ \lambda \circ \iota$ is a polarization of $Y$. In particular,  $Z\cap Y$ is finite. Now the kernel of the  homomorphism $Y\times Z\to X$ in consideration is contained in $(Y \cap Z)\times (Y\cap Z)$ and therefore finite. The proposition follows from theorem (\ref{thm isogenies}) part c).
	\end{proof}
...
%	as $\mathbb{Q}$-algebras.
	...
	\end{align*}
	\end{defn}
	It follows from theorem (\ref{thm isogenies}) that $T_l X$ is (non-canonically) isomorphic to $\mathbb{Z}_l^{2g}$ and we introduce the $2g$ dimensional $\mathbb{Q}_l$ vector space $V_l(X) := T_l(X)\otimes_{\mathbb{Z}_l} \mathbb{Q}_l =T_l (X)\otimes_{\mathbb{Z}_l} (\mathbb{Z}_l\otimes_{\mathbb{Z}} \mathbb{Q})= T_l(X)\otimes_{\mathbb{Z}} \mathbb{Q}  $. 
	
	A homomorphism of abelian varieties $f: X \to Y$ induces a homomorphism $T_l f : T_l X \to T_l Y$. It sends a point $(0, x_1, x_2, \dots )\in T_l X$ to $(0, f(x_1), f(x_2), \dots)\in T_l Y$. It follows from the definition that this is functorial. In particular, $\End_k(X)\to \End_{\mathbb{Z}_l}(T_l X), f \mapsto T_l f$, as well as, \begin{align}V_l: \End_k(X)\otimes_{\Z} \mathbb{Q} \to \End_{\mathbb{Q}_l}(V_l X), f\otimes c\mapsto c\cdot(T_l(f)\otimes_{\Z_l}\id_{\Q_l})\label{eq vl is algebra homo} 
	\end{align} are algebra homomorphisms. 
	\begin{rmk}\label{rmk tate as hom}
	$\mathbb{Q}_l/\mathbb{Z}_l$ is the union of its subgroups $l^{-n}\mathbb{Z}_l/\mathbb{Z}_l$, which we identify with $\mathbb{Z}/l^n \mathbb{Z}$. Therefore as rings $\mathbb{Q}_l/\mathbb{Z}_l =\colim (\mathbb{Z}/l^n \mathbb{Z})$, where the colimit is taken over the homomorphisms $\mathbb{Z}/l^n \mathbb{Z} \hookrightarrow \mathbb{Z}/l^{n+1}\mathbb{Z}$ given by $(1 \mod l^n) \mapsto (l \mod l^{n+1})$ and we see that $T_l X = \Hom(\mathbb{Q}_l/\mathbb{Z}_l, X(k_s))$. Using this characterization of the Tate module and the long exact sequence of $\text{Ext}_\mathbb{Z}(\mathbb{Q}_l/\mathbb{Z}_l, \cdot )$ modules it can be shown that for any isogeny $f: X \to Y$ the induced map $T_l f: T_l X \to T_l Y$ is injective with cokernel isomorphic to the $l$-Sylow group of $(\ker f)(k_s)$ and further that $V_l f: V_l X \to V_l Y$ is an isomorphism, see \cite[Cor. 10.7]{van2007abelian}.  
	\end{rmk}
	...
	For $f\in \End(X)$ we define $\deg f$ to be the degree of $f$ if $f$ is an isogeny and zero otherwise. Because the degree is multiplicative we can extend this to $\deg:\End^0(X)\to \Q$ via $\deg(\frac{f}{n}):=n^{-2g}\deg{f}$. 
 	...
 	There exists isogenies $X\to \prod_{i=1}^r X_i$ and $Y\to  \prod_{j=1}^n Y_j$, where the $X_i, Y_j$ are simple abelian varieties. Proposition \ref{prop division} lets us map $\Hom(X,Y)$ into $\Hom(\prod_{i=1}^r X_i^{m_i}, \prod_{i=1}^r Y_i^{n_i})$ and since $n_X$ is an epimorphism for all $n \in \Z$ this is injective. Since every nonzero homomorphisms of simple abelian varieties is an isogeny, $\Hom(\prod_{i=1}^r X_i^{m_i}, \prod_{i=1}^r Y_i^{n_i})=\prod_{i,j}\Hom(X_i, Y_j)$ and if $X_i$ and $Y_i$ are isogenous then $\Hom(X_i , Y_i) $ embeds into $\End(X_i)$, else $\Hom(X_i , Y_i) =0$. So, the theorem follows from the special case proven above.
  	\end{proof}
 	\begin{cor}\label{cor bound dimension homo}	
 		$\Hom^0(X,Y):=\Hom_k(X,Y)\otimes \Q$ has $\Q$ dimension $\le 4 (\dim X )(\dim Y)$. 
 	\end{cor}
 	\begin{proof}
 	For an abelian variety $X$ the $\Z_l$-module $T_l X$ is free of rank $2\cdot \dim X$ and therefore $\Hom_{\Z_l}(T_l X, T_l Y)$ is free of rank $4(\dim X)( \dim Y)$. Since $\Z_l$ is a principal ideal domain we can conclude from theorem \ref{thm injec} that $\Z_l \otimes \Hom(X,Y)$ is a free $\Z_l$ module of rank $\le 4(\dim X)( \dim Y)$.  This bounds the rank of the torsion free abelian group $\Hom(X,Y)$ by $4\dim X \dim Y$. 
 	\end{proof}
 	...
 	\begin{thm}\label{thm charak poly} For $f \in \End^0(X)$ let $P_{f,l}\in \Q_l[x]$ be the characteristic polynomial of $V_l f  \in \End_{\Q_l}(V_l X)$. Then $P_{f,l}=P_f$ is independent of $l$ and has integer coefficients whenever $f \in \End(X)$. 
 	\end{thm}
 	\begin{proof} We only give a sketch, whereas a detailed proof can be found in \cite[Chapt. 12]{milne1986abelian} or \cite[thm. 12.8]{van2007abelian}. It can be assumed that $f\in \End(X)$ and, further, using corollary \ref{cor decomposition simple} that $X$ is simple. 
 		...
 		Set $g=\id$.
 		We start with the notation of the proof of theorem \ref{thm polynomial mapping} for a chosen ample symmetric divisor $D$ and interchange the roles of $f$ and $g$. Lemma \ref{lem pullbacks along multiplication} shows that $D'\sim -2 D$ and we conclude from the last equation in the proof of theorem  \ref{thm polynomial mapping} that $P_f$ has integer coefficients and leading coefficient $1$.
...
A regular local ring is reduced and therefore $\Pic^0_{C/k}$ is geometrically reduced over $k$. Moreover, we have seen in section \ref{sec sum} that $\Pic^0_{C/k}$ is also proper and geometrically irreducible, i.e. $\Pic^0_{C/k}$ is an abelian variety. We will refer to $J:=\Pic^0_{C/k}$ as \textit{Jacobian variety} or short \textit{Jacobian} of $C$. By theorem \ref{thm tangent space of Picard Functor} $J$ has dimension $g$ and its tangent space at the zero is isomorphic to $H^1(C, \Om_C)$. In particular, if $g=0$ then $J=\Spec(k)$.
 	...
 	\subsection{The canonical map from $C$ to its Jacobian}\label{sec canonical map}
 	   ...
 	   \begin{align}
  		...
  		\end{proof}
  	\begin{rmk}[\textit{Elliptic Curves}]\label{rmk elliptic curves}
 	...
 	For $m_1, \dots, m_k \in \mathbb{N}_0$ a partition $n=m_1+\dots+m_r$ the natural isomorphism $C^{m_1}\times \dots\times C^{m_r}\to C^{n}$ induces a natural morphism $s=s_{m_1,\dots,m_r}: C^{(m_1)}\times \dots \times C^{(m_r)} \to C^{(n)}$ that we will refer to the sum map.
 ...
 In particular, $\pi: C^n \to C^{(n)}$ is finite and flat of degree $n!$. 
   ...
   We obtain a contravariant functor $\Div: \Sch_{/k} \to \text{Sets}$ with
	...
	Via the description of $f^{(n)}$ in (\ref{eq on points 18}) in the case that $T=k$, we see that the $k$-valued points of the fiber of $f^{(n)}$ containing $D\in C^{(n)}(k)$ will correspond to the complete linear system $\abs{D}$. The set $\abs{D}$ is in natural bijection with $(\Gamma(X, \Om(D))\setminus \{0\})/k^\times$ via $D+(f)\mapsto \{\lambda f \mid \lambda \in k^\times\}$. This observation on $k$-valued points has a scheme-theoretic reformulation.
	
 	\begin{thm}[Abel's theorem]\label{thm Abels theorem}
 		Let $\Li$ be a line bundle of degree $n$ on $C$. Then the scheme-theoretic fiber of $f^{(n)}: C^{(n)}\to J$ over the point  $p\in J(k)$ represented by $\Li\otimes \Li(P)^{-n}\in \Pic^0(C)$  is \[{f^{(n)}}^{-1}(p)=\mathbb{P}(H^0(C, \Li)):=\text{Proj}(\text{Sym}(H^0(C, \Li)))\cong \mathbb{P}_k^m,\] for $m=h^0(C, \Li)-1$. 
 	\end{thm}
 	\begin{proof}
...
\begin{proof} 
	...
	For proving surjectivity, it suffices to show that the map is surjective on $k$ valued points. Let $n\ge g$. Using that for $\Li\in \Pic(C\times T)$ the degree function $T\ni t \mapsto \deg(\Li\vert_{C\times \{t\}})$ is locally constant, it can be shown that $J(k)$ can be identified with $\Pic^0(C)$, the degree $0$ line bundles on $C$, see \cite[14.1]{van2007abelian}. For any $x\in J(k)$ represented by $\Li \in \Pic^0(C)$ the Riemann-Roch theorem implies that $\Li\otimes \Li(P)^n$ is effective, and therefore $x$ is in the image of $f^{(n)}$.
	...
	\end{align} Thus $[K-E]$ is effective. Choose a point $Q\in C(k)$ which is not a base point of the linear system $\abs{K-E}$. Then $h^0(K-E-Q)=h^0(K-E)-1\overset{\ref{eq comput h}}{=}g-n $, where the first equality follows from $[K-E]$ being effective and because there is no meromorphic function $f\in K(C)$, whose only pole is $Q$, since else $f$ would define an isomorphism $C\to \mathbb{P}^1$. 
...
Consider the Mumford line bundle $\Lambda(\Li(\Theta))=m^*\Li(\Theta) \otimes \pr_1^*\Li(\Theta)^{-1}\otimes \pr_2^*\Li(\Theta)^{-1}$ on $J\times J$ from section \ref{sec dual abelian}. We obtain a $k$-morphism $\varphi_{\Li(\Theta)}: J \to J^\vee$ with $(1\times \varphi_{\Li(\Theta)})^*\Po \cong \Lambda(\Li(\Theta))$.
...
\begin{rmk}\label{rmmk alter varphi by translation}
	It is shown in \cite[14.22 and 14.28 (ii)]{van2007abelian}  that $\Theta$, $\Theta^-$ and $\Theta_a$ are numerically equivalent and this implies that $\varphi_{\Li(\Theta)}=\varphi_{\Li(\Theta^-)}=\varphi_{\Li(\Theta_a)}$ for all $a\in J(k)$ by \cite[7.26]{van2007abelian}.
	
	In particular, by lemma \ref{lem pullbacks along multiplication} the divisors $n^*\Theta $ and $n^2\Theta$ are numerically equivalent for all $n\in \mathbb{Z}$.
\end{rmk}
...
and $\mathcal{M}^P\vert_{C\times \{a\}}$ are isomorphic on $C$. By the so-called Seesaw principle \cite[5.1]{milne1986abelian}, which is a theorem on proper varieties, there exists an invertible sheaf $\mathcal{F}$ on $J$ such that 
...
Hence $-\varphi_{\Li(\Theta)}\circ f^\vee=\id_{J^\vee}$. This is by definition of  $\mathcal{M}^P$ as the universal line bundle on $C\times J$ and the uniqueness assertion in proposition \ref{prop poincare}. By theorem \ref{thm isogenies} both $\varphi_{\Li(\Theta)}$ and $f^\vee$ are isogenies. Now their degree must be equal to one and the theorem follows from proposition \ref{prop division}.
\end{proof}
...
\end{proof}
\subsection{The Rosati involution}\label{sec rosati}
...
Let $\sigma:C\times C \to C\times C$ be the $k$ morphism that switches the factors of $C\times C$. Then $\sigma$ acts on 
\[F:=\{\Li\in \Pic(C\times C): \Li\vert_{C\times \{P\}} \text{ and } \Li\vert_{\{P\}\times C} \text { is trivial }\}\]
...
We invoke intersection theory on the surface $C\times C$. Notation will be as in Hartshorne \cite[chap. V.1]{hartshorne2013algebraic} but we won't assume that $k$ is necessarily algebraically closed. For effective divisors $D, C$ on $C\times C$ we define their intersection number $C.D:=\deg_C \Li_{C\times C}(D)\vert_C$ and extend this definition to a symmetric, bilinear map $\text{Div}(C\times C)\times \text{Div}(C\times C)\to \mathbb{Z}$ that only depends on the linear equivalence class of the inputs. To see that there exists a unique such pairing consult \cite[V1, Thm. 1.1 ]{hartshorne2013algebraic}.
%
...
\begin{proof}[Sketch of a proof]
	...
	By remark \ref{rmmk alter varphi by translation} $D'$ is numerically equivalent to $2D$ and, so, $D_n$ is numerically equivalent to $n^2D +n D_\Theta(h)+h^*D$. 
	...
	Now it only remains to justify $(f(C)\cdot D_\Theta(h))=\deg f^*\Li(D_\Theta(h))$. This is \cite[Exmp. 7.1.17]{fulton2013intersection}.
\end{proof}
...
\end{align*}
So, by the previous lemma \ref{lem trace from intersection},  $\tr(h)=\deg f^*D_\Theta(h) =\deg \Delta_C^*\Li(X)^{-1}=\Delta_C.(-X)=-\Delta_C.X$.
\end{proof}
\begin{cor}[Positivity of the Rosati involution]\label{cor Positiviy of the Rosati involutionn}
	Let $h, g\in \End(J)$ then 
	\[\tr(h^\dagger \circ g)=\deg((h\circ f,g\circ f)^*\Lambda(\Li(\Theta)))\text{ and } \tr(h^\dagger\circ h)=2\deg((h\circ f)^*\Li(\Theta))=2((h\circ f)(C)\cdot \Theta).\]
	The \textit{trace form} 
	\[\End^0(J)\times \End^0(J) \to \mathbb{Q}, \;(g,h)\mapsto \tr(g\circ h^\dagger) \]
	is bilinear, symmetric and positive definite.
\end{cor}
\begin{proof}
	It can be read of the proof of lemma \ref{lem adjoint of map} that $(1_C\times h^\dagger)^*\M^p\cong ((h\circ f)\times 1)^* \Lambda(\Li(\Theta)^{-1})$. So, by the Lefschetz trace formula \ref{thm Lefschetz trace formula}, 
	\begin{align*}
		\tr(h^\dagger \circ g)&=-\deg \Delta_C^*(1_C \times (h^\dagger\circ g \circ f)) =-\deg \Delta_C^*((h\circ f) \times (g\circ f))^* \Lambda(\Li(\Theta)^{-1})\\&=\deg((h\circ f,g\circ f)^*\Lambda(\Li(\Theta))).
	\end{align*}
	In particular, $\tr(h^\dagger, h)=\deg((h\circ f, h\circ f)^*\Lambda(\Li(\Theta)))$. We compute 
	\begin{align*}(h\circ f, h\circ f)^*\Lambda(\Li(\Theta))&=(h\circ f, h\circ f)^*(m^*\Li(\Theta)\otimes \pr_1^*\Li(\Theta)^{-1}\otimes \pr_2^* \Li(\Theta)^{-1})\\&\cong (h\circ f)^*(2_J)^*\Li(\Theta)-(h\circ f)^*\Li(\Theta)^2
	\end{align*}
	and, since after taking degree only the numerical equivalence class of $\Theta$ matters, we obtain \[\deg((h\circ f, h\circ f)^*\Lambda(\Li(\Theta))))=2^2\deg((h\circ f)^*\Li(\Theta))-2\deg((h\circ f)^*\Li(\Theta))\]
	by remark \ref{rmmk alter varphi by translation}.
	 So, $\tr(h^\dagger \circ  h)=2\deg((h\circ f)^*\Li(\Theta))=2((h\circ f)(C)\cdot \Theta)$, where the last equality is by \cite[Exmp. 7.1.17]{fulton2013intersection}. 
	 
	 By lemma \ref{lem rosati involution} and theorem \ref{thm Lefschetz trace formula} $\tr(h^\dagger)=-\Delta_C.\sigma^* X = -\Delta_C.X=\tr(h)$. In particular, the trace form is symmetric. Bilinearity follows from linearity of the Rosati involution and the properties of the trace. 
	 	 
	 If $h\neq 0$, then $Y:=(h\circ f)(C)$ is a nontrivial integral closed subscheme of dimension $1$ on $J$. By the Nakai-Moishezon criterion for ampleness, the intersection number $(Y\cdot \Theta)$ is positive. In other words, $\tr(h^\dagger\circ h)>0$. 
\end{proof}
\subsection{The map induced on the Jacobian by an endomorphism of $C$}\label{sec induced map}
...
\begin{lem}\label{lem adjoint of map}
...
\end{proof}
 We know try to relate $\tr(\alpha')=\tr(\alpha*)$ with the fixed points of $\alpha$. 

\begin{thm}[Lefschetz fixed point formula]\label{thm Lefschetz fixed point formula}
	We have $\Delta_C. \Gamma_{\alpha}=1-\tr(\alpha^*)+\deg(\alpha)$.
\end{thm}
\begin{proof}
	By lemma \ref{lem adjoint of map} we have $(1\times (\alpha^*\circ f))^*\M^P \cong \Li(\Gamma_{\alpha}-C\times \{\alpha(P)\}-\alpha^{-1}(P)\times C)$ and so by the Lefschetz trace formula \ref{thm Lefschetz trace formula} 
	\begin{align*}
		\tr(\alpha^*)=-\Delta_C.(\Gamma_{\alpha}-C\times \{\alpha(P)\}-\alpha^{-1}(P)\times C)=-\Delta_C.\Gamma_{\alpha} +1+\Delta_C.(C\times \alpha^{-1}(P)).
	\end{align*}
	Since $\Delta_C$ and $C\times \alpha^{-1}(P)$ have no components in common and have scheme-theoretic intersection $\alpha^{-1}(P)$ we conclude $\Delta_C.(C\times \alpha^{-1}(P))=h^0(\alpha^{-1}(P), \Om_{\alpha^{-1}(P)} )$. Because $\alpha$ is flat and finite, $\deg \alpha=h^0(\alpha^{-1}(P), \Om_{\alpha^{-1}(P) })$ by exactly the same proof as  in theorem \ref{thm isogenies}. 
\end{proof}
\begin{exm}
	Applying the Lefschetz fixed point formula to $\id_C$ yields $\Delta_C^2=1-2g-1$.
\end{exm}
The interpretation of theorem \ref{thm Lefschetz fixed point formula} as fixed point formula is justified by the following proposition. 
\begin{prop}\label{prop fixed points and intersection}
	Assume $k$ to be algebraically closed.
	Let $\alpha:C\to C$ be non-constant such that \begin{enumerate}[a)]
		\item $\alpha(x)=x$ for only finitely many $x\in C(k)$ and
		\item $\dd_x \alpha \neq \id_{T_x C}$ for all $x\in C(k)$ with $\alpha(x)=x$, 
	\end{enumerate}
	then $\Gamma_\alpha. \Delta_C =\#\{x\in C(k): \alpha(x)=x\}$. 
\end{prop}
\begin{proof}
	Condition a) implies $(\Delta_C \times_{C\times C}\Gamma_{\alpha})(k)=\{x\in C(k) : \alpha(x)=x\}<\infty$.\\ So, $\Gamma_\alpha \cap \Delta_C:=\Gamma_\alpha\times_{C\times C} \Delta_C $ is finite and $\Gamma_\alpha$ and $\Delta_C$ have no components in common. Hence $\Gamma_{\alpha}.\Delta_C =\dim_k \Gamma(\Gamma_\alpha \cap \Delta_C, \Om_{\Gamma_\alpha \cap \Delta_C})$. 
	
	Let $i: \Delta_C \to C$ and $j: \Gamma_\alpha \to C$ be the inclusion. Then for $P \in (\Gamma_\alpha \cap \Delta_C)(k)$ we have $\text{im} (\dd i)_P =\text{span}(\pi,\pi)$ and $\text{im} (\dd j)_P =\text{span}(\pi,\dd \alpha_P(\pi))$, where $\pi$ is a generator of $T_p(C)$ and we identify $T_P(C\times C)$ as $T_P(C)\oplus T_P(C)$. Hence by assumption b) $T_P(C\times C)=\text{im}(\dd i)_P + \text{im}(\dd j)_P$. 
	
	Let $f$ be the local equation for $\Delta_C$ in $C\times C$ at $P$ and $g$ the local equation for $\Gamma_{\alpha}$ in $C\times C$ at $P$. Then $T_P(C\times C)=\text{im}(\dd i)_P + \text{im}(\dd j)_P$ shows that $f$ and $g$ generate $m_P$ at $\Om_{C\times C, P}$. Hence $\Om_{\Gamma_{\alpha}\cap \Delta_C, P}=\Om_{C\times C, P}/(f,g)=k$ and therefore 
	\[ \Gamma_{\alpha}.\Delta_C=\dim_k \Gamma(\Gamma_\alpha \cap \Delta_C, \Om_{\Gamma_\alpha \cap \Delta_C})=\#(\Gamma_\alpha \cap \Delta_C)(k)=\{x\in C(k): \alpha(x)=x\}.\]	
\end{proof}
\section{The Weil conjectures for curves}
...
Let $J$ be the Jacobian variety of $C$ over $\mathbb{F}_q$. Then the base change of $J$ to $k$, denoted by $J_k$, is the Jacobian variety of $C_k$. Further, the induces map on $J_k$ by $F_{C,k}$ as in section \ref{sec induced map} a) is $\text{Fr}:=F_J\times 1_k$, where $F_J$ is the absolute Frobenius of $J$. This follows from the naturality of the absolute Frobenius. 

\begin{thm}[Weil conjectures for curves]\label{thm Weil conjectures for curves} ~\\
	Let $P\in \mathbb{Z}[x]$ be the characteristic polynomial of $\text{Fr}\in \End(J_k)$ and $\alpha_1, \dots, \alpha_{2g} \in \mathbb{C}$ its roots. The Weil conjectures for curves state:
	\begin{enumerate}
		\item (\textit{Rationality of the zeta function}) \[\exp\left( \sum_{n=1}^\infty \# C(\mathbb{F}_{q_n}) \frac{x^n}{n}\right) =\frac{\prod_{i=1}^{2g}(1-\alpha_ix)}{(1-x)(1-qx)}\]
		and $\prod_{i=1}^{2g}(1-\alpha_i x)=x^{2g}P(\frac{1}{x})\in \mathbb{Z}[x]$. 
	...
	\begin{align} \#\Pic^0(C_{\mathbb{F}_{q^n}})= \prod_{i=1}^{2g}(1-\alpha_i^n), \label{eq1}
	\end{align}
where $\Pic^0(C_{\mathbb{F}_{q^n}})$ denotes the group of isomorphism classes of degree zero line bundles on the curve  $C_{\mathbb{F}_{q^n}}:=C\times_{\mathbb{F}_q} \mathbb{F}_{q^n}$ over $\mathbb{F}_{q^n}$.
\end{thm}
\begin{proof} Let $F_k: \Spec(k)\to\Spec(k)$ denote the absolute Frobenius of $k$. 
	
	The Lefschetz fixed point formula \ref{thm Lefschetz fixed point formula} applied to $F_{C,k}^n$ yields \[\Delta_C.\Gamma_{F_{C,k}^n}=1-\tr(\text{Fr}^n)+\deg(F_{C,k}^n).\] 
	Using theorem \ref{thm charak poly} we have $\tr(\text{Fr}^n)=\sum_{i=1}^{2g} \alpha_i^n$. Further, $\deg(F_{C,k}^n)=\deg(F_{C,k})^n$.
	
	\textbf{Claim 1:} $\deg(F_{C,k})=q$.
	By \cite[8.5 Prop. 13]{bosch2013algebraic} there is a nonempty open neighborhood $U\subseteq C_k$ and an étale $k$- morphism $g:U\to \mathbb{A}^1_k$.  It suffices to prove that $F_{C,k}\vert_U: U\to U$ has degree $q$. Now $g\circ F_{C,k}=(F_{\mathbb{A}^1_{\mathbb{F}_q}}\times 1_k) \circ g$. By the multiplicativity of the degree it is enough to prove that $\deg(F_{\mathbb{A}^1_{\mathbb{F}_q}}\times 1_k)=q$ (Note that $g$ induces a finite residue field extension). To see this, observe that $F_{\mathbb{A}^1_{\mathbb{F}_q}}$ fixes coefficients and maps coordinates to their $q$-th power. The induced residue field extension is $\mathbb{F}_q(x)\hookrightarrow \mathbb{F}_q(x)[t]/(t^q-x)$, which has degree $q$. 
	
	So, we obtain
	\begin{align}\Delta_C.\Gamma_{F_{C,k}^n}=1-\sum_{i=1}^{2g}\alpha_i^{n}+q^n. \label{eq intersection frobenius diagonal}
	\end{align}
	...
	\begin{align}C_k(k)\to C(k),\; (x: k \to C_k)\mapsto (k\xrightarrow{x}C_k \xrightarrow{\pr_C}. C) \label{eq bijection}\end{align}
	\begin{enumerate}[a)]
	\item $C(k)$ has an action given by pre-composition with $F_k$. 
	
	\item $C_k(k)$ has an action given by post-composition with $F_{C,k}$. 
\end{enumerate}
\textbf{Claim 2}:	The bijection in (\ref{eq bijection}) identifies both actions a) and b):\\ Let $x\in C_k(k)$. Then $\pr_C\circ F_{C,k} \circ x=F_C \circ \pr_C \circ x=F_S\circ \pr_C \circ x$, where we used the naturality of the absolute Frobenius in the last equation.
	
	Let $C(k)^{F_k^n}$ denote the elements of $C(k)$ fixed by pre-composition with $F_k^n$. \\
	\textbf{Claim 3: } $C(\mathbb{F}_{q^n})\to C(k)^{F_k^n}, (x: \Spec(\mathbb{F}_{q^n})\to C)\mapsto (\Spec(k)\to \Spec(\mathbb{F}_{q^n})\xrightarrow{x}C )$ is a bijection.
	
	The map is injective, because $\Spec(k)\to \Spec(\mathbb{F}_{q_n})$ is faithfully flat and therefore an epimorphism.
	Next we show surjectivity. Say, $y\in C(k)^{F_k^n}$. Take any $\Spec(R)=U\subseteq C$ open affine such that $y$ factors as $k\to U\hookrightarrow C$. Then $k\to U$ corresponds to a $\mathbb{F}_q$ algebra homomorphism $\varphi: R\to k$ such that $\mathfrak{f}\circ \varphi=\varphi$, for $\mathfrak{f}(r)=r^{q^n}$. This implies that $\varphi$ factors through $\mathbb{F}^{q^n}\hookrightarrow k$. Hence, $y$ factors as $\Spec(k)\to \mathbb{F}_{q^n}\to U\hookrightarrow C$. 
	
	If we denote elements of $C_k(k)$ fixed by post-composition with $F_{C,k}^n$ with $C_k(k)^{F_{C,k}^n}$, then applying claim 3 and then claim 2 shows that \begin{align}\#C(\mathbb{F}_{q^n})=\# C(k)^{F_k^n}=\#C_k(k)^{F_{C,k}^n}=\# \{x\in C_k(k): F_{C,k}^n(x)=x\}.\label{eq fixed points froben}
	\end{align}
	Next we want to apply proposition \ref{prop fixed points and intersection} to $F^n_{C,k}: C_k \to C_k$. 
	
	For part a) of proposition \ref{prop fixed points and intersection} note that $\#C(\mathbb{F}_{q^n})<\infty$ because $C$ admits a closed immersion into $\mathbb{P}^m_k$ for $m$ big enough and $\mathbb{P}^m_k(\mathbb{F}_{q^n})=((q^n)^{m+1}-1)/(q^n-1)<\infty$. So, by equation (\ref{eq fixed points froben}) $F_{C,k}^n(x)=x$ for only finitely many $x\in C_k(x)$. 
	
	For part b) of proposition \ref{prop fixed points and intersection} observe that for $x\in C_k(k)$ we have $\dd_x F_{C,k} =0$, because $q^n=0$ in $k$. This can be proven very explicitly by using an étale morphisms as in the proof of claim 1. Then  $\dd_x F_{C,k} =0$ follows because the endomorphism of $\mathbb{A}_{F_q}$ that fixes the coefficients and raises the coordinates to their $q^n$ power induces the zero map on tangent spaces. 
	
	All in all, we can apply proposition \ref{prop fixed points and intersection} to obtain
	\begin{align}\#C(\mathbb{F}_{q^n})\overset{(\ref{eq fixed points froben})}{=}\#\{x \in C_k(k): F_{C,k}^n(x)=x\}\overset{\ref{prop fixed points and intersection} }{=}\Delta_C.\Gamma_{F_{C,k}^n}\overset{(\ref{eq intersection frobenius diagonal})}{=}1-\sum_{i=1}^{2g}\alpha_i^n + q^n. \label{eq points of C in proof}
	\end{align}  
	For showing the Riemann hypothesis for the curve $C$ we start with the following claim.\\
	\textbf{Claim 4:} $\text{Fr}^\dagger \circ \text{Fr}=q_J$, or in other words $  \text{Fr}^\vee \circ \varphi_{\Li(\Theta)}\circ \text{Fr} =q_J\cdot \varphi_{\Li(\Theta)}$. 
	
	Let $P\in J$ and let $g$ be a local equation cutting out  $\Theta$ on the neighborhood $U$ of $P$. Note that, since $\Theta$ can be defined over $\mathbb{F}_q$, we have $\text{Fr}_U^\#(g)= g^{q}$ and this is a local equation cutting out $\text{Fr}^*\Li(\Theta)$ at $P$. But $\text{div}(g^q)=q\cdot \text{div}(g)$ and thus $\text{Fr}^*\Li(\Theta)\cong \Li(q\Theta)\cong \Li(\Theta)^{q}$. 
	
	Now given a $k$-valued point $x\in J_k(k)$ we can compute via equation (\ref{eq dual map on points}) and (\ref{eq square}) that $\text{Fr}^\vee \circ \varphi_{\Li(\Theta)}\circ \text{Fr}(x)$ is represented by the line bundle 
	\begin{align*}
		\text{Fr}^*(t_{\text{Fr}(x)}^*\Li(\Theta)\otimes \Li^{-1}(\Theta))\cong t^*_x \text{Fr}^*\Li(\Theta)\otimes (\text{Fr}^*\Li(\Theta))^{-1}\cong t_x^*\Li(\Theta)^q\otimes (\Li(\Theta)^q)^{-1}=\varphi_{\Li(\Theta)^{q}}(x).
	\end{align*}
But $\varphi_{\Li(\Theta)^{q}}=q\varphi_{\Li(\Theta)}$ because $\Li'\mapsto \varphi_{\Li'}$ is a homomorphism, see equation (\ref{eq varphi}). So, $  \text{Fr}^\vee \circ \varphi_{\Li(\Theta)}\circ \text{Fr} $ and $q_J\cdot \varphi_{\Li(\Theta)}$ agree on $k$-valued points. Since $k$ is algebraically closed, claim 4 follows.\\
\textbf{Claim 5:} Every complex root $\alpha$ of $P$ has absolute value $\abs{\alpha}=\sqrt{q}$.

Note that $\mathbb{Q}[\text{Fr}]\subseteq \End^0(J_k)$ is a commutative ring. 
Further $\End^0(J_k)$ is finite dimensional as $\mathbb{Q}$-vector space by corollary \ref{cor bound dimension homo} and therefore $\mathbb{Q}[\text{Fr}]$ is a finite commutative $\mathbb{Q}$-algebra. 
 
By the relation $\text{Fr}^{-1}=\text{Fr}^\dagger/q$ from claim 4 we see that $\text{Fr}$ is not a zero divisor in $\End(J_k)^0$. Hence the $\mathbb{Q}$ linear endomorphism of $\mathbb{Q}[\text{Fr}]$ given by multiplication by $\text{Fr}$ is injective. Because $\mathbb{Q}[\text{Fr}]$ is finite dimensional the endomorphism is also surjective and therefore $\Fr^{-1}\in \mathbb{Q}[\text{Fr}]$. Again by the relation in claim 4 we obtain $\text{Fr}^\dagger\in \mathbb{Q}[\Fr]$. So, by the properties of the Rosati involution from section \ref{sec rosati} $(\cdot )^\dagger: \mathbb{Q}[\text{Fr}]\to \End^0(J_k)$ maps into $\mathbb{Q}[\text{Fr}]$. 

Let $0\neq a \in \Q[\Fr]$. Define $b:=a^\dagger \cdot a$. By the positivity of the Rosati involution \ref{cor Positiviy of the Rosati involutionn} $\tr(b)=\tr(a^\dagger \cdot a)>0$. So, $b\neq 0$. As $b^\dagger = b$ also $\tr(b^2)=\tr(b^\dagger b)>0$, again by the positivity of the Rosati involution \ref{cor Positiviy of the Rosati involutionn}, and hence $b^2\neq 0$. Similarly, $b^4\neq 0$ and by induction $b^{2m}\neq 0 $ for all $m\ge 0$. Hence $b$ is not nilpotent. Because $\Q[\Fr]$ is commutative, if $a$ was nilpotent, then also $b$ would be nilpotent. We conclude that $\Q[\Fr]$ is reduced. 

Since $\Q[\Fr]$ is a finite commutative  $\mathbb{Q}$-algebra, i.e. Artinian, it has a finite number of prime ideals $\m_1, \dots, \m_j$ each of which is maximal. Since $\mathbb{Q}[\Fr]$ is reduced, we see that $\bigcap_{i=1}^{j}\m_i=0$. So, by the chinese reminder theorem $\mathbb{Q}[\Fr]\cong \prod_{i=1}^j K_i$ for $K_i:= \mathbb{Q}[\Fr]/\m_i$ a field. Any automorphism $\tau$ of $\Q[\Fr]$ maps a maximal ideal to a maximal ideal, i.e. there is a permutation $\sigma \in S_j$, for $S_j$ the symmetric group in $j$-letters, and  isomorphisms $\tau_i: K_i \to K_{\sigma(i)}$ such that $\tau(a_1,\dots, a_j)=(b_1, \dots, b_j)$ for $b_{\sigma(i)}=\tau_i(a_i)$ and $a_i, b_i \in K_i$. The Rosati involution is an automorphism of $\mathbb{Q}[\Fr]$ as we have seen that it restricts to a map  $\mathbb{Q}[\Fr]\to \mathbb{Q}[\Fr]$, is linear and its own inverse. Further, $(\cdot)^\dagger$ is multiplicative because $\mathbb{Q}[\Fr]$ is commutative. For $\tau=\dagger$ the permutation $\sigma$ from above must be trivial: Else, for $\sigma(i)\neq i$ and $a:=(0,\dots, 0,1,0 \dots 0)\in \mathbb{Q}[\Fr]$, $1$ in the $i$-th spot, $\tr(\alpha^\dagger \cdot \alpha)=0$, which contradicts corollary \ref{cor Positiviy of the Rosati involutionn}. 

Hence $\dagger$ preserves the factors of $\mathbb{Q}[\Fr]$ and is a positive-definite involution on each
of them. The involution extends by linearity (equivalently by continuity) to a positive-definite involution of $\mathbb{Q}[\Fr]\otimes \mathbb{R}$, i.e. $(\cdot )^\dagger: \mathbb{Q}[\Fr]\otimes \mathbb{R} \to \mathbb{Q}[\Fr]\otimes \mathbb{R}$ is an $\mathbb{R}$-automorphism that is its own inverse and there exists $\tr: \mathbb{Q}[\Fr]\otimes \mathbb{R}\to \mathbb{R}$, which is $\mathbb{R}$-linear such that $\tr(a^\dagger \circ a)>0$ for all $a\in \mathbb{Q}[\Fr]\otimes \mathbb{R}$.

The above remarks also apply to $\mathbb{Q}[\Fr]\otimes \mathbb{R}$: it is a finite $\mathbb{R}$ algebra and a product of
fields, where $\dagger: \mathbb{Q}[\Fr]\otimes \mathbb{R} \to \mathbb{Q}[\Fr]\otimes \mathbb{R} $ is a positive-definite involution that preserves each factor of $\mathbb{Q}[\Fr]\otimes \mathbb{R}$.

Since any finite field extension of $\mathbb{R}$ is either $\mathbb{R}$ itself or isomorphic to $\mathbb{C}$, each factor of $\mathbb{Q}[\Fr]\otimes \mathbb{R}$ is either $\mathbb{R}$ or $\mathbb{C}$.

The field $\mathbb{R}$ has no nontrivial automorphisms at
all, and so $(\cdot)^\dagger$ must act on a real factor of $\mathbb{Q}[\Fr]\otimes \mathbb{R}$ as the identity map. 

The field $\mathbb{C}$ has only
two automorphisms of finite order: the identity map and complex conjugation.
The identity on $\mathbb{C}$ is not positive definite, else $0<\tr( i \cdot i)=\tr(-1)=-1\tr(1)$, which contradicts that $\tr(1)=\tr(1\cdot 1)>0$. Hence, $(\cdot)^\dagger$ must act on the complex factors as conjugation. 

Now given any homomorphism of commutative $\mathbb{Q}$-algebras $\rho:\Q[\Fr]\to \mathbb{C}$. \\Then $\rho\otimes 1:\Q[\Fr]\otimes \mathbb{R}\to \mathbb{C}\otimes_\mathbb{Q}\mathbb{R}$ is $\mathbb{R}$ linear, so for any $a\in \Q[\Fr]$
\begin{align*}
	\rho(a^\dagger)\otimes 1&=(\rho\otimes 1)(a^\dagger \otimes 1)=(\rho\otimes 1)((a\otimes 1)^\dagger) =(\rho\otimes 1)(\overline{a\otimes 1})\\&=\overline{(\rho\otimes 1)(a\otimes 1)}=\overline{\rho(a)\otimes 1}=\overline{\rho(a)}\otimes 1
\end{align*}
where $\overline{a\otimes 1}$ denote complex conjugation in every coordinate of $a\otimes 1=(a_1,\dots, a_j)\in \Q[\Fr]\otimes \mathbb{R}$. \\So , by flatness of the $\mathbb{Q}$ module $\mathbb{Q}[\Fr]$ we have $\rho(a^\dagger)=\overline{\rho(a)}$. 

Let $\mu\in \mathbb{Q}[x]$ be the unique polynomial with leading coefficient $1$ that generators the kernel of $\mathbb{Q}[x]\to \mathbb{Q}[\Fr], x\mapsto \Fr$. Then the map $ \Q[x]/(\mu) \to \mathbb{Q}[\Fr], x+(\mu)\mapsto \Fr$ is an isomorpism.

Let $\alpha\in \mathbb{C}$ be a root of $\mu$. The ring-homomorphism $\psi:\Q[x]\to \mathbb{C}, \;f\mapsto f(\alpha)$  factors as $\mathbb{Q}[x]\to \mathbb{Q}[x]/(\mu) \xrightarrow{x+(\mu)\mapsto \alpha }\mathbb{C}$. So, we obtain a ring-homomorphism $\rho: \mathbb{Q}[\Fr]\to \mathbb{C}$ sending $\Fr$ to $\alpha$. 
This yields  $\abs{\alpha}^2=\overline{\alpha}\cdot \alpha= 
\overline{\rho(\Fr)}\rho(\Fr)=
\rho(\Fr^\dagger)\rho(\Fr)= \rho(\Fr^\dagger \circ \Fr) \overset{\text{ Claim 4}}{=}\rho(q)=q$. 

Now let $l$ be a prime $l\neq \text{char}(k)$. Then the minimal polynomial $\Tilde{\mu}$ of $V_l(\Fr)\in \End_\mathbb{Q}(V_l(J_k))$ divides $\mu$, because $\mu(V_l(\Fr)))\overset{(\ref{eq vl is algebra homo})}{=}V_l(\mu(\Fr)))=V_l(0)=0$. Hence, all roots of $\Tilde{\mu}$ are roots of $\mu$ and therefore have absolute value $\sqrt{q}$. But the characteristic polynomial of $V_l(\Fr)$ has the same roots as $\Tilde{\mu}$, so claim 5 follows by theorem \ref{thm charak poly}.

Equation (\ref{eq points of C in proof}) in conjunction with the Riemann hypothesis give the Hesse-Weil bound:
\begin{align*}
	\abs{\#C(\mathbb{F}_{q^n}-(q^n+1))}=\abs{\sum_{i=1}^{2g}\alpha_i^n}\le \sum_{i=1}^{2g}\abs{\alpha_i}^n=2g(\sqrt{q})^n.
\end{align*}

The identity $\ln(1-t)=-\sum_{n=1}^\infty \frac{t^n}{n}$ implies that 
\begin{align*}
	\ln(\frac{\prod_{i=1}^{2g}(1-\alpha_ix)}{(1-x)(1-qx)})&=-\ln(1-x)+\sum_{i=1}^{2g} \ln(1-x\alpha_i)-\ln(1-qx)\\&=\sum_{n=1}^\infty\left( \frac{x^n}{n}\right)-\sum_{i=1}^{2g}\left(\sum_{n=1}^\infty \frac{x^n\alpha_i^n}{n}\right)+\sum_{n=1}^\infty \left( \frac{(qx)^n}{n}\right)\\&=\sum_{n=1}^\infty \left(\left(1-\sum_{i=1}^{2g}\alpha_i^n+q^n\right)\frac{x^n}{n}\right)=\sum_{n=1}^{\infty}\left(\#C(\mathbb{F}_{q^n})\frac{x^n}{n}\right),
\end{align*}
where we used equation (\ref{eq points of C in proof}) for the last equality. This proves the rationality of the zeta function. Moreover, $\prod_{i=1}^{2g}(1-\alpha_ix)=x^{2g}\prod_{i=1}^{2g}(\frac{1}{x}-\alpha_i)=x^{2g}P(\frac{1}{x})$ has integer coefficients, because $P\in\mathbb{Z}[x]$  by theorem \ref{thm charak poly}. 

\textbf{Claim 6:} The map $1-\Fr^n:J_k\to J_k$ is an étale morphism: Since $k$ is algebraically closed it suffices to proof that $1-\Fr^n$ induces an isomorphism on tangent spaces for all $k$-valued points. But we have already seen that $\Fr^n$ induces the zero map on tangent spaces, hence $1-\Fr^n$ is the identity on tangent spaces and claim 6 follows. 

In particular, $1-\Fr^n$ is an isogeny by theorem \ref{thm isogenies} and by equation (\ref{eq3})  $h^0(\ker(1-\Fr^n), \Om_{\ker(1-\Fr^n)})=\deg(1- \Fr^n)$. Now by claim 6 the fiber $\ker(1-\Fr^n)$ is reduced and finite over the algebraically closed field $k$, so $\#\ker(1-\Fr^n)(k)=h^0(\ker(1-\Fr^n), \Om_{\ker(1-\Fr^n)})$. All in all, we obtain
\begin{align*}
 \#\{x\in J_k(k): \Fr^n(x)=x\}= \#\ker(1-\Fr^n)(k)=h^0(\ker(1-\Fr^n), \Om_{\ker(1-\Fr^n)})=\deg(1- \Fr^n).
\end{align*}
Note that claim 2 and claim 3 can be analogously be proven for the absolute Frobenius $F_J:J\to J$ and its base change $\Fr=F_J\times 1_k:J_k=J\times_{\mathbb{F}_q}k\to J_k$.
So, applying equation (\ref{eq fixed points froben}) to the variety $J_k=J\times_{\mathbb{F}_{q}}k$ shows that $\# J(\mathbb{F}_{q^n})=\#\{x\in J_k(k): \Fr^n(x)=x\}$.

Now $\# J(\mathbb{F}_{q^n})=\# J_{\mathbb{F}_{q^n}}(\mathbb{F}_{q^n})$ and the latter equals $\# \Pic^0(C_{\mathbb{F}_{q^n}})$ whenever $C_{\mathbb{F}_{q^n}}$ has an $\mathbb{F}_{q^n}$-valued point by equation (\ref{eq 9}) and the discussion in section \ref{sec induced map} b). This uses that  $J_{\mathbb{F}_{q^n}}=J\times_{\mathbb{F}_q}\mathbb{F}_{q^n}$ is the Jacobian variety of the curve $C_{\mathbb{F}_{q^n}}$ over $\mathbb{F}_{q^n}$. 

Let $P_{\Fr^n}$ be the characteristic polynomial of $\Fr^n$. By theorem \ref{thm charak poly} the roots of $P_{\Fr^n}$ are $\alpha_1^n, \dots, \alpha_{2g}^n$. Further, by definition  of  $P_{\Fr^n}$ we have $P_{\Fr^n}(1)=\deg(1-\Fr^n)$.

All in all, for $n$ big enough, i.e. such that $C(\mathbb{F}_{q^n})\neq \emptyset$, 
\begin{align*}
	\# \Pic^0(C_{\mathbb{F}_{q^n}})=\# J_{\mathbb{F}_{q^n}}(\mathbb{F}_{q^n})=\#\{x\in J_k(k): \Fr^n(x)=x\}=\deg(1-\Fr^n)=P_{\Fr^n}(1)=\prod_{i=1}^{2g}(1-\alpha_i^n).
\end{align*} 
This prove equation (\ref{eq1}) and therefore completes the proof.
\end{proof}
\begin{exm}[Elliptic Curves]
	Suppose the genus of $C$ is equal to one. Then the Hesse-Weil bound gives us that $\# C(\mathbb{F}_q)=1-(\alpha_1+\alpha_2)+q$ and by the Riemann-Hypothesis $\abs{\alpha_1+\alpha_2}\le 2 \sqrt{q}$. Therefore, $\# C(\mathbb{F}_q)\ge 1-2\sqrt{q}+q=(\sqrt{q}-1)^2$. Hence $C$ will admit an $\mathbb{F}_q$ valued point, i.e. $C$ is an elliptic curve, see remark \ref{rmk elliptic curves}.
\end{exm}
