...
\end{enumerate}
	The bijection above identifies both actions: Let $x\in C_k(k)$. Then $\pr_C\circ F_{C,k} \circ x=F_C \circ \pr_C \circ x=F_S\circ \pr_C \circ x$, where we used the naturality of the absolute Frobenius in the last equation.
	
	Let $C(k)^{F_k^n}$ denote the elements of $C(k)$ fixed by pre-composition with $F_k^n$. \\
	\textbf{Claim: } $C(\mathbb{F}_{q^n})\to C(k)^{F_k^n}, (x: \Spec(\mathbb{F}_{q^n})\to C)\mapsto (\Spec(k)\to \Spec(\mathbb{F}_{q^n})\xrightarrow{x}C )$ is a bijection.
