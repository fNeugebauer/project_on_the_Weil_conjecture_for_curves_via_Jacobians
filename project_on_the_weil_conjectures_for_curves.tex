...
\end{proof}
\begin{cor}[Positivity of the Rosati involution]
	Let $h, g\in \End(J)$ then 
	\[\tr(h^\dagger \circ g)=\deg((h\circ f,g\circ f)^*\Lambda(\Li(\Theta)))\text{ and } \tr(h^\dagger\circ h)=2\deg((h\circ f)^*\Li(\Theta))=2(f(C)\cdot h^*(\Theta)).\]
...
\begin{proof}
	...
	by remark \ref{rmmk alter varphi by translation}.
	 So, $\tr(h^\dagger \circ  h)=2\deg((h\circ f)^*\Li(\Theta))=2\deg(f^*h^*\Theta))=2(f(C)\cdot h^*\Theta)$, where the last equality is by \cite[Exmp. 7.1.17]{fulton2013intersection}. 
	 
	 By lemma \ref{lem rosati involution} and theorem \ref{thm Lefschetz trace formula} $\tr(h^\dagger)=-\Delta_C.\sigma^* X = -\Delta_C.X=\tr(h)$. In particular, the trace form is symmetric. Bilinearity follows from linearity of the Rosati involution and the trace. 
	 
	 If $h\neq 0$, then $h\circ f: C\to h(J)$ is finite, where $h(J)$ denotes the scheme-theoretic image of $h$ considered as abelian variety. (This is theorem \ref{thm isogenies} and \ref{thm closed immer}). The restriction of $\Theta$ to $h(J)$ is ample by \cite[\href{https://stacks.math.columbia.edu/tag/0892}{Tag 0892}]{stacks-project} and corollary \ref{cor properties of principal polarization of jacobian}. The pullback of an ample line bundle along a finite morphism is again ample by the same reference. So, $(h\circ f)^*\Li(\Theta)$ is ample and in particular has positive degree and therefore $\tr(h\circ h^\dagger)=\tr(h^\dagger \circ h)=2\deg((h\circ f)^* \Li(\Theta))>0$. 
	 This proves that the trace form is positive definite. 
\end{proof}
